...
class FamilyDetails extends Component {
  ...
  constructor(props) {
    super(props);
     ...
     this.state = {alertDialogFlag:false , disabled : false , flag : "0" , errors : {} , dbUserId : props.id, info:[{"createdBy" : UserContext.userid ,"updatedBy" : UserContext.userid , "isActive" : "Y" , "dbUserId" : props.id}],
         ...
         if(jsondata.appError==null){      
            ...
            // alert(JSON.stringify(jsonobjects));
             ...
             else 
             {
              ...
              alertDialogOptions.message=<span style={{color:"green"}}>Familk
              y Details Updated Sucessfully</span>;
              this.setState({alertDialogFlag:true});
              setTimeout(() => {this.props.history.push({
