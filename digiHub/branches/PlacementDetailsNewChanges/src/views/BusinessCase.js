...
fetchUserDocuments(UserContext.centerId == 760 ?this.state.linkedEngagementId:this.state.engagementId,'Business Brief','P').then((jsondata) => {   
...
fetchBusinessCaseMetaData(UserContext.centerId == 760 ?this.state.linkedEngagementId:this.state.engagementId).then((jsondata) => { 
   // if(jsondata.appError==null){    
    let jsonobjects = JSON.parse(jsondata.data);  
...
let localThis=this;
  ...
  // formData.append('data','{"token" : "", "action" : "captureDocDetails", "data" : [{"dbUserId":'+localThis.state.dbUserId+',"engagementId":'+localThis.state.engagementId+',"documentType":"Business Brief","typeOfDocument":"P","documentName":"Business Brief","base64File":"'+document+'"}]}');
  // // fetch(serviceEndPoint.uploadServiceEndPoint, {
  //   fetch(process.env.REACT_APP_API_ENDPOINT+"ssservices-v1/sservices-v1/documents", {
  //     method: 'post',
  //     headers: {
