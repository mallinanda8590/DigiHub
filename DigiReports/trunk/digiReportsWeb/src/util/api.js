...
export async function setDefaultSettings(userId,roleMapId,centerId) {
    if(isSessionValid()){
    let requestFormData = new FormData();
    requestFormData.append('data', '{"token" : "", "action" : "setDefaultSettings", "data" : [{"userId":'+userId+',"roleMapId":'+roleMapId+',"centerId":'+centerId+'}]}');
    if(!isTokenValid()) 
        await regenerateToken();
     return fetch(serviceEndPoint.userProfileServiceEndPoint, {
        method: "POST",
...
export async function searchByFirstNameAndLastNameAnddobAndPrimaryContactNumberAndPrimaryEmailId(firstName,lastName,dob,primaryContactNumber,primaryEmailId,highestQualification) {
    if(isSessionValid()){
    let requestFormData = new FormData();
    requestFormData.append('data', '{"token" : "", "action" : "searchByFirstNameAndLastNameAnddobAndPrimaryContactNumberAndPrimaryEmailId", "data" : [{"firstName":"'+firstName+'","lastName":"'+lastName+'","dob":"'+dob+'","highestQualification":"'+highestQualification+'","primaryContactNumber":"'+primaryContactNumber+'","primaryEmailId":"'+primaryEmailId+'"}]}');
    if(!isTokenValid()) 
        await regenerateToken();
   return fetch(serviceEndPoint.studentServiceEndPoint, {
        method: "POST",
...
//code for fetch all details of student digiprofile

export async function fetchAllStudentDetails(studentId) {
    if(isSessionValid()){
    let requestFormData = new FormData();  
    requestFormData.append('data', '{"token" : "", "action" : "fetchAllStudentDetails", "data" : [{"studentId":'+studentId+'}]}');
    if(!isTokenValid()) 
        await regenerateToken();
...
export async function fetchAllStudentAssessmentDetails(dbUserId) {
    if(isSessionValid()){
    let requestFormData = new FormData();  
    requestFormData.append('data', '{"token" : "", "action" : "getData", "data" : [{"dbUserId":'+dbUserId+'}]}');
    if(!isTokenValid()) 
        await regenerateToken();
    return  fetch(serviceEndPoint.assessmentDataService,{
     method: "POST",
     headers: {
        'Authorization': 'Bearer '+UserContext.token
    }, 
...
//code for post placement details
export async function fetchAllPostPlacementDetails(id) {
    if(isSessionValid()){
    let requestFormData = new FormData();  
    requestFormData.append('data', '{"token" : "", "action" : "getData", "data" : [{"id":'+id+'}]}');
    if(!isTokenValid()) 
        await regenerateToken();
    return  fetch(serviceEndPoint.postPlacementDataService,{
     method: "POST",
     headers: {
        'Authorization': 'Bearer '+UserContext.token
    }, 
...
export async function fetchUserProfileBydbUserIdAndTypeOfDocument(dbUserId,typeOfDocument,status,documentType) {
    if(isSessionValid()){
    let requestFormData = new FormData();
    requestFormData.append('data', '{"token" : "", "action" : "viewProfilepicture", "data" : [{"dbUserId":'+dbUserId+',"typeOfDocument":"'+typeOfDocument+',"status":'+status+',"documentType":'+documentType+'"}]}');
    if(!isTokenValid()) 
        await regenerateToken();
  return fetch(serviceEndPoint.documentServiceEndPoint, {
        method: "POST",
