...
class ExistingBusiness extends Component {
    constructor(props) {
        ...
        this.state = {existingBusinessQuestions:[],answers:[],
            ...
            city:[],
            village:[],
            address : {id:'',createdBy : UserContext.userid,'isActive' : 'Y','type' : 'EB' , pincode : "" , addressLine1 : "" , addressLine2 : "" , district : "" , state : "" , cityName : "", villageName : ""},
  ...
  for(var i=0;i<answers.length;i++){ 
   if(answers[i].questionId==row.id && answers[i].response==radioValue && radioValue.includes("Yes")){        
     subQuestions.forEach(function(subQuestion) {
        console.log("Why it is enabled now");
...
return    <SingleSelect  isClearable={true}  name={row.id} id={row.id}
          options={data} onChange={this.handleSelectChange.bind(this,row.id)} value={this.displaySelectedData(row.data,row.id,"drop down")}/>
}

else if (row.userInteraction=="text box"){
return  <TextField   type="text" name = {row.id} id = {row.id}  onChange={this.handleInputChange.bind(this)}  value={this.displaySelectedData('',row.id,"text box")} />
}
...
console.log("alreadyAnswered is"+alreadyAnswered);

const array = [...this.state.answers];

  if(!alreadyAnswered){
  console.log("data is adding to object");
    var data= {"questionId":name,"response":value,"studentId":this.state.dbUserId,"engagementId":this.state.engagementId}
    array.push(data);
...
saveExistingBusiness = (event) => {
    ...
    // var uniqueArray=this.getUnique(this.state.answers);
