...
import {checkButton,validateToRecommend, getBusinessCaseDocument , getSocioEconomicData, getInterestInventory,getExperienceDetails, getBasicDetails, getAddressData , getFamilyData, getObservationData, getBusinessCaseData} from './../util/validation';
...
class Recommend extends Component
{
    constructor(props) {
       super(props);
        ...
        this.state = {valid : true , res : {} , selectedIndex : [] , rowsSelected : {'dbUserId' : '','studentId':'', 'engagement_id' : ''},disabled : false , flag : "0" ,errors : {},bieData:[], info:[], engagementData : {},engagementId : props.engagementId,dbUserId : props.id, studentId :props.id, recommendData : {action : "" ,remarks : "" , actionedBy : UserContext.userid ,roleid:UserContext.roleid,roleName:UserContext.roleName}}
    ...
    componentDidMount()
    {
      if(UserContext.roleid == 4)
      {
        ...
        getSocioEconomicData(this.state.engagementId , this.state.studentId).then(result => this.setState({
          res : {
...
render()
{
  ...
  if(UserContext.roleid == 4)
  {
    ...
    {
      res.push("SocioEconomic details incomplete");
    }
