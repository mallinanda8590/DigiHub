...
import { fetchBatchDetailsForBatchOwner ,fetchEnrollmentDetailsByBatchId,
  fetchAllStudentDataByEngagementId,fetchAllEngagementForStudent,captureStudentEngagementDetails,fetchBatchDetailsByBatchId} from './../util/api';
...
shortList = (e) => {
 ...
 this.setState({alertDialogFlag: false });
  ...
  if(dbUserId){
    ...
    checkEligibleForShortlist(dbUserId).then(result => result == 'true' ? this.alertNotEligible() : captureStudentEngagementDetails(dbUserId,UserContext.centerId,UserContext.userid).then((jsondata) => { 
...
{/* 
            { (this.state.selectedBatchDetails!=null) && 
             <div> <span style={{fontWeight: 'bold'}}> Name </span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].batchName} <br></br>
             <span style={{fontWeight: 'bold'}}> Description</span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].batchDescription}<br></br>
             <span style={{fontWeight: 'bold'}}> Start Date </span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].startDate}<br></br>
             <span style={{fontWeight: 'bold'}}> End Date </span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].endDate}<br></br>
             <span style={{fontWeight: 'bold'}}> Ojt Start Date</span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].ojtStartDate}<br></br>
             <span style={{fontWeight: 'bold'}}> Ojt End Date </span>&nbsp;&nbsp;: {this.state.selectedBatchDetails[0].ojtEndDate}      
             </div>
            }   
                */}
               <br/> 
