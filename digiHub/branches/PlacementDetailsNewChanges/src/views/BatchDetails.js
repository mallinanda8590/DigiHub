...
import { FormControl , InputLabel, Input, Grid , TextField, Hidden}  from '@material-ui/core';
import Button from '@material-ui/core/Button';
import { passwordReset, fetchUsersByCenterId, fetchUserDetailsById, saveBatchDetails,fetchCourseDetails,fetchCenterCapacity,
  fetchCenterActiveCourses,fetchCourseDetailsByIds,fetchCenterCapacityByIds } from './../util/api';
import {validateDates,  validateOJTDates , validateBatchDescription,validateStartDate, roleBasedReadonly,validateAssessmentDate,validateOJTEndDate} from './../util/validation';
  ...
  constructor(props) {
    ...
    if(props.location.state != null && props.location.state != undefined)
    {
      this.state = {startDate:new Date(props.location.state.batch.startDate).toISOString().slice(0,10),endDate:new Date(props.location.state.batch.endDate).toISOString().slice(0,10)
        ,traingHrs:0,Ojt:0,courseData:[],user:[],errors : {courseId:'',batchType:'',batchDescription:'',batchOwner:'',ydmCoach:'',startDate:'',endDate:'',ojtStartDate:'',assessmentDate:'',ojtEndDate:'',batchHelperId:''},batchData : props.location.state.batch 
    };
   // alert("get data"+JSON.stringify(this.state.batchData));

    }
    else
    
    {
      this.state ={startDate:new Date().toISOString().slice(0,10),endDate:new Date().toISOString().slice(0,10),freezeDate:'',traingHrs:0,Ojt:0,courseData:[], user:[],errors : {courseId:'',batchType:'',batchDescription:'',batchOwner:'',ydmCoach:'',startDate:'',endDate:'',ojtStartDate:'',assessmentDate:'',ojtEndDate:'',batchHelperId:''},batchData : {courseId:'',batchType:'',batchDescription:'',batchOwner:'',ydmCoach:'',startDate:'',endDate:'',ojtStartDate:'',createdBy : UserContext.userid, dbUserId:UserContext.dbUserId,updatedBy:UserContext.userid,
      assessmentDate:'',ojtEndDate:'',batchHelperId:''
      }};
...
if (batchOwner === batchHelperId){
  this.state.errors.batchHelperId="Batch Helper and Batch Owner not same";
}


if (ydmCoach === batchHelperId){
  this.state.errors.ydmCoach="YDM Coach and Batch Owner not same";
 }
...
getFreezeDate(startDate){
 ...
 this.setState({freezeDate:freezeDate});
...
<FormControl>
 ...
 { 
  (this.state.freezeDate) &&         
  <div style={{color:"red"}}>Batch Freeze Date is : {this.state.freezeDate} </div>
}
